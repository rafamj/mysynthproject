...
AlsaOut::AlsaOut():Device(si,2,0,0,0,0,0,0){
  deviceName="default";
}

AlsaOut::AlsaOut(const char *devName):Device(si,2,0,0,0,0,0,0){ //left right 
  deviceName=devName;
  //open();
}

void AlsaOut::open(){
    handle=alsaOpen(deviceName,SND_PCM_STREAM_PLAYBACK,Device::rate,256,4,2);
    /*
...
void AlsaOut::callback(void (*cycle)()){
    ...
    while(current<avail*2){
      ...
      //printf("%d %d\n",getInput(0),NORMALIZE(getInput(0)));
      buffer[current++]=CONVERT16(getInput(0));
      //printf("callback2\n");
      //buffer[current++]=getInput(1)*16*256;
      buffer[current++]=CONVERT16(getInput(1));
      cycle();
      //printf("callback3\n");
      //devices->cycle();
      //printf("callback4\n");
